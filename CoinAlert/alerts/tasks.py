...
@periodic_task(run_every=(crontab(minute="*/3")), name="time_alert", ignore_result=True)
def time_alert():
    ...
    if queryset.exists():
        ...
        for alert in queryset:
            ...
            before_url = before_base_url.format(
                base_asset, quote_asset, before_date.replace(microsecond=0).isoformat()
            )
...
@periodic_task(
    run_every=(crontab(minute="*/3")), name="value_alert", ignore_result=True
)
def value_alert():
    ...
    if queryset.exists():
        ...
        for alert in queryset:
            ...
            if (criteria == "ABV") and (api_value >= alert_value):
                ...
                user_email = user.email
                subject = (
                    f"CoinAlert 🔔 : {base_asset} is above {alert_value} {quote_asset} !"
                )
            ...
            if (criteria == "BLW") and (api_value <= alert_value):
                ...
                user_email = user.email
                subject = (
                    f"CoinAlert 🔔 : {base_asset} is below {alert_value} {quote_asset} !"
                )
